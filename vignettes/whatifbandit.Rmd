...
summary(sim, level = 0.8) |>
  select(AIPW, SE, lower_bound, upper_bound, level) |>
  print(width = Inf)
# By hand
quantile <- qnorm(0.2 / 2, lower.tail = FALSE)
sim_summary |>
  mutate(
    lower_bound = AIPW - SE * quantile,
...
Using future comes with some new complexities. Generally, Windows users should use `plan("multisession")`, while Linux and MacOS users should use `plan("multisession")` or `plan("multicore")`. If your using a High Performance Computing Cluster, the future.batchtools API should be used, with the proper plan for your HPC scheduler. A full treatment of futures it beyond the scope of this vignette. For details on how futures work, and what future plans are available, please consult the [future](https://future.futureverse.org/) documentation.
