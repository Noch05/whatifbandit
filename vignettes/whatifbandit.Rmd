...
time <- system.time(
  multiple_sims <- multiple_mab_simulation(data = tanf,
                      assignment_method = "Date",
                      time_unit = "Month",
                      period_length = 1,
                      algorithm = "Thompson",
...
Executing those 100 simulations took `r as.numeric(time["elapsed"])` seconds. To speed up this process `multiple_mab_simulation()` supports parallel processing via the furrr and future packages. To run the function in parallel, all you need to do is call `future::plan()` before running it and specify the number of cores you want to use.
```{r}
#| echo: FALSE
...
Using future comes with some new complexities. Windows users must use `plan("multisession")`, while Linux and MacOS users
can use `plan("multisession")` or `plan("multicore")`. If your using a High Performance Computing Cluster, the future.batchtools API should be used instead to specify the proper plan for the scheduler of your HPC. See the future, furrr, and future.batchtools package documentation for more details on parallel processing.
...
```
`type = hist` creates histograms showcasing the distribution for the AIPW, Sample, or both estimates for each
treatment arm, over the repeated trials.
```{r}
#| fig.width: 5
plot(multiple_sims, type = "hist", estimator = "AIPW", bins = 50)
```

`type = estimate` is the same as for `mab`, but now the spread can be shown using either empirical or normal CDF.
```{r}
#| fig.width: 5
plot(multiple_sims, type = "estimate", estimator = "AIPW", cdf = "empirical", level = 0.99, height = 0.4) + 
  scale_x_continuous(breaks = seq(0, 1, .1), limits = range(0, 1))
```
Once again, all the data used to construct these is available in the output, so you can perform your own custom
analysis, or make more detailed plots by hand. 
...
```{r}
#| eval: FALSE
dataframe_time <- system.time(single_mab_simulation(data = as.data.frame(tanf_large),
                      assignment_method = "Batch",
                      period_length = 3000,
                      algorithm = "Thompson",
                      conditions = levels(tanf$condition),
                      ...
                      prior_periods = "All",
                      blocking = FALSE, 
                      data_cols = c(id_col = "id",
                                    success_col = "success",
                                    condition_col = "condition")
                      ))

datatable_time <- system.time(single_mab_simulation(tanf_large,
                      assignment_method = "Batch",
                      period_length = 3000,
                      algorithm = "Thompson",
                      conditions = levels(tanf$condition),
...
```
The data.table version completes in `r datatable_time` while the data.frame version takes r ` dataframe_time` making the the data.table version `r ((dataframe_time/datatable_time) - 1)*100`% faster in this case. 

The data.table version may not be faster in all cases. Performance is mainly dependent on the size of the data, the number of observations in each period, and the total number of periods. For simulations where each period contains < 500 observations or the total size is < 1,000,000 rows, the data.table version will likely be slower. There is no hard and fast rule, you must make the judgement based on the data you have, the simulation you want to run, and the computing power of your system.

The Thompson Sampling Corrections may introduce new errors, that we have not caught yet. If you get an error from an internal function call, we reccomend switching to UCB1 which does not have the same issues, and reporting bug as an issue on package's [GitHub Repo](https://github.com/ryantmoore/whatifbandit).

### Error Handling
`single_mab_simulation()` and `multiple_mab_simulation()` can have more than 20 arguments, so its easy to make a typo when calling the function. As a result, we provide helpful error messages to help you deduce what has gone wrong with your execution of the function. If a required column is not properly entered, or any of the arguments are invalid, a helpful error will be thrown to notify the user of any issues.
...
```{r}
try(multiple_mab_simulation(data = tanf,
                      assignment_method = "Batch",
                      period_length = 1000,
                      algorithm = "Thompson",
                      conditions = levels(tanf$condition),
