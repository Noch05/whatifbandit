...
Here we've kept `keep_data = TRUE` to showcase the memory impact of saving the data from each iteration. The full object size with all the data is `r as.numeric(object.size(multiple_sims, units = "MiB"))` megabytes (MiB). However, when we remove the data from the object, the size becomes just `r multi_sims$final_data_nest <- NULL; as.numeric(object.size(multiple_sims, units = "MiB"))` megabytes(MiB).
